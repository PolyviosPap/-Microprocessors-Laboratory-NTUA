...
int main()
{
	...
	while(1) {
		...
		// check if BTN0 is pressed or not.
		buttons_value = XGpio_DiscreteRead(&buttons_inst, 1);
		check = (buttons_value & 1);

		xil_printf("buttons value: %d\r\n", buttons_value);
		for (delay = 0; delay < LED_DELAY; delay++);

		if (check != 1) {
			// if not pressed, move the led.

			if (dir_flag == 0) {
				// must move towards left.
				led_value = (led_value << 1);

				if (led_value == 8) {
					// an ftasame sto terma, allakse kateythinsi.
					dir_flag = 1;
				}
				XGpio_DiscreteWrite(&leds_inst, 1, led_value = led_value);
				for (delay = 0; delay < LED_DELAY; delay++);
